...
#define MPU9250_IMU_SLAVE_ADDR 0x69
...
static uint8_t
twi_read_byte (twi_t twi, uint8_t slave_addr, uint8_t addr)
{
    uint8_t response = 0;
    uint8_t status;
...
static uint8_t
twi_write_byte (twi_t twi, uint8_t slave_addr, uint8_t addr, uint8_t value)
{
    return twi_master_addr_write (twi, slave_addr,
                                  addr, 1, &value, 1);
}


static uint8_t
mpu9250_imu_read_byte (mpu9250_t *mpu9250, uint8_t addr)
{
    return twi_read_byte (mpu9250->twi, MPU9250_IMU_SLAVE_ADDR, addr);
}


static uint8_t
mpu9250_imu_write_byte (mpu9250_t *mpu9250, uint8_t addr, uint8_t value)
{
    return twi_write_byte (mpu9250->twi, MPU9250_IMU_SLAVE_ADDR, addr, value);
}


static uint8_t
mpu9250_mag_read_byte (mpu9250_t *mpu9250, uint8_t addr)
{
    return twi_read_byte (mpu9250->twi, MPU9250_MAG_SLAVE_ADDR, addr);    
}


static uint8_t
mpu9250_mag_write_byte (mpu9250_t *mpu9250, uint8_t addr, uint8_t value)
{
    return twi_write_byte (mpu9250->twi, MPU9250_MAG_SLAVE_ADDR, addr, value);
}
...
{
    return (mpu9250_imu_read_byte (mpu9250, MPU9250_IMU_INT_STATUS) & 0x01) != 0;
}


static bool
mpu9250_imu_data_read (mpu9250_t *mpu9250, uint8_t addr, int16_t data[3])
{
    uint8_t rawdata[6];
    twi_ret_t status;

    status = twi_master_addr_read (mpu9250->twi, MPU9250_IMU_SLAVE_ADDR,
                                   addr, 1, rawdata, 6);
...
{
    return mpu9250_imu_data_read (mpu9250, MPU9250_IMU_ACCEL_XOUT_H, acceldata);
}
...
{
    return mpu9250_imu_data_read (mpu9250, MPU9250_IMU_GYRO_XOUT_H, gyrodata);
}
...
{
    return (mpu9250_mag_read_byte (mpu9250, MPU9250_MAG_ST1) & 0x01) != 0;
}
...
mpu9250_mag_data_read (mpu9250_t *mpu9250, int16_t magdata[3])
{
    ...
    status = twi_master_addr_read (mpu9250->twi, MPU9250_MAG_SLAVE_ADDR,
...
static bool
mpu9250_imu_init (mpu9250_t *mpu9250)
{
    uint8_t response;

    /* This should read 0x71.  */
    response = mpu9250_imu_read_byte (mpu9250, MPU9250_IMU_WHO_AM_I);

    if (response != 0x71)
        return 0;    

    mpu9250_imu_write_byte (mpu9250, MPU9250_IMU_PWR_MGMT_1, 0x00);
    DELAY_US (100);

    /* Choose best clock source.  */
    mpu9250_imu_write_byte (mpu9250, MPU9250_IMU_PWR_MGMT_1, 0x01);
    DELAY_US (200);

    mpu9250_imu_write_byte (mpu9250, MPU9250_IMU_CONFIG, 0x03);

    mpu9250_imu_write_byte (mpu9250, MPU9250_IMU_SMPLRT_DIV, 0x04);

    /* Enable I2C bypass so commands can be sent to magnetometer.  */
    mpu9250_imu_write_byte (mpu9250, MPU9250_IMU_INT_PIN_CFG, 0x02);    
...
static bool
mpu9250_mag_init (mpu9250_t *mpu9250)
{
    uint8_t response;

    /* This should read 0x48.  */
    response = mpu9250_mag_read_byte (mpu9250, MPU9250_MAG_DEVICE_ID);
    if (response != 0x48)
        return 0;
...
mpu9250_init (void)
{
    static mpu9250_t imu_dev;
    mpu9250_t *mpu9250 = &imu_dev;
    static bool init = 0;

    if (!init)
    {
        mpu9250->twi = twi_init (&twi_cfg);
        init = 1;
    }

    if (! mpu9250_imu_init (mpu9250))
        return 0;

    if (! mpu9250_mag_init (mpu9250))
        return 0;            
